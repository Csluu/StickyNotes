...
import { getFormattedDate, generateRandomId } from "../../utils/noteUtils";
...
export default function NoteWindow() {
	...
	useEffect(() => {
		...
		const handler = (event, receivedId) => {
			setNoteId(receivedId);
		};
	...
	const handleTextChange = useCallback(
		(text) => {
			if (!note) return;
			// Don't setNote here — it would re-trigger initialValue on EditableText
	...
	return (
		<main className="bg h-full w-full">
			<div className="flex w-full h-full flex-col place-items-center">
				<div className="no-drag flex w-full h-full flex-row place-items-start justify-center p-5">
					<section
						id="noteBody"
						className="note-card yes-drag relative flex w-screen h-full flex-col gap-2  overflow-y-auto rounded-3xl border-[1px] border-black bg-clear-dark-gray px-5 pb-8 shadow-border"
					>
						<header className="mt-6 flex flex-row justify-between gap-2">
							...
							<div className="no-drag flex flex-row items-start">
								{/* Preview toggle button */}
								<button
									onClick={handleTogglePreview}
									className="no-drag flex h-8 w-8 place-items-center justify-center rounded-lg"
								...
								>
									{previewMode ? (
										...
										<svg
											...
											height="18"
											viewBox="0 0 24 24"
											className="opacity-50 hover:opacity-100 hover:scale-110"
										>
											<path
												fill="gray"
								...
								<DropdownMenu>
									<ul className="text-sm text-gray-200 gap-3 pb-3 pt-3">
										<DropdownItem onClick={handleOpenMain}>
											<svg
												...
												height="16"
												viewBox="0 0 24 24"
												className=" opacity-50 duration-150  ease-in-out group-hover:scale-105 group-hover:opacity-100"
											>
												<path
													fill="gray"
											...
											<span className="duration-150 ease-in-out group-hover:scale-[102%]">
												Main Menu
											</span>
										</DropdownItem>
										...
										<DropdownItem onClick={handleToggleLock}>
											<svg
												...
												height="16"
												viewBox="0 0 24 24"
												className="duration-150  ease-in-out group-hover:scale-105 group-hover:opacity-100 opacity-50 "
											>
												<path
													fill="gray"
													d={
											...
											<>
												<DropdownItem onClick={handleRestore}>
													...
													>
														<path
															fill="gray"
															d="M12.5 3C7.81 3 4 6.81 4 11.5S7.81 20 12.5 20h.5V18H12.5C8.92 18 6 15.08 6 11.5S8.92 5 12.5 5C14.73 5 16.65 6.27 17.82 8.08L16.5 9.5H20V6l-2.5 2.25C16.41 6.89 14.54 6 12.5 6 9.46 6 7 8.46 7 11.5c0 1.18.38 2.29 1.07 3.19L7 16h4v4h2v-4h4l-1.5-1.85C18.5 12.69 19 11.58 19 10.5 19 6.91 15.5 3 12.5 3Z"
														/>
													</svg>
													<span className="duration-150 ease-in-out group-hover:scale-[102%]">
														Restore
										...
										<DropdownItem onClick={handleQuit} danger>
											<svg
												...
												viewBox="0 0 16 16"
												version="1.1"
												className="duration-150 opacity-50 ease-in-out group-hover:scale-105 group-hover:opacity-100"
											>
												<path
													style={{ fill: "gray" }}
						...
						<EditableText
							key={note.id} // ← forces a full remount when note changes
