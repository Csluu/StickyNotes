...
export default function NoteCard({ note, onClick, searchQuery = "", isArchived = false }) {
  const handlePin = useCallback(
    (e) => {
      e.stopPropagation();
      const updated = { ...note, pinned: !note.pinned };
      window.electronAPI?.saveNote?.(updated) || window.electronAPI?.updateNote?.(note.id);
    ...
    >
      ...
      <header className="flex flex-row items-center justify-between gap-2 card-text w-full">
        <div className="flex flex-row items-center gap-2 min-w-0 flex-1">
          <span className={`${note.color || "bg-green-500"} w-3 h-3 rounded-full flex-shrink-0`} />
